function ComposeTweetAssistant(opts, assistant) {
		this.textarea = 'txtCompose',
		this.completebar = 'complete-bar';
		...
		if (opts.reply_id) {
			...
			get('submit').update('Post Reply');
		}

		if (opts.dm) {
			get('dm').addClassName('show');
			get('submit').update('Send Message');
		}
	...
	ComposeTweetAssistant.prototype = {
		setup: function() {
	var prefs = new LocalStorage();
		...
		Mojo.Event.listen(get('submit'), Mojo.Event.tap, this.submitTweet.bind(this));
		Mojo.Event.listen(get('photo'), Mojo.Event.tap, this.photoTapped.bind(this));
		Mojo.Event.listen(get('geotag'), Mojo.Event.tap, this.geotagTapped.bind(this));
		Mojo.Event.listen(get('link'), Mojo.Event.tap, this.linkTapped.bind(this));
		Mojo.Event.listen(get('cancel'), Mojo.Event.tap, this.cancelTapped.bind(this));
		Mojo.Event.listen(get('complete-bar'), Mojo.Event.tap, this.addUser.bind(this));
},
...
updateCounter: function() {
				...
				callback: function() {
					...
					var sendfunc = function()
					{
						...
						if (!msg) {
							...
							if (!this.rt) {
								//close the card ... work in progress
							} else {
								//close the card ... work in progress
							}
							return;
	...
	geotagTapped: function(event) {
		if (!this.geo) {
			...
			this.lat = 0;
			this.lng = 0;
			this.controller.get('geotag').removeClassName('active');
	...
	gotLocation: function(response){
		...
		this.controller.get('geotag').addClassName('active');
		banner('Found you!');
	},
...
cleanup: function(event) {
	get(this.textarea).stopObserving('click');
		...
		Mojo.Event.stopListening(get('submit'), Mojo.Event.tap, this.submitTweet);
		Mojo.Event.stopListening(get('photo'), Mojo.Event.tap, this.photoTapped);
		Mojo.Event.stopListening(get('geotag'), Mojo.Event.tap, this.geotagTapped);
		Mojo.Event.stopListening(get('link'), Mojo.Event.tap, this.linkTapped);
		Mojo.Event.stopListening(get('cancel'), Mojo.Event.tap, this.cancelTapped);
		Mojo.Event.stopListening(get('complete-bar'), Mojo.Event.tap, this.addUser);
	},
};